...
# STRUCTURE:
#     - Two temperature treatments: Cold, warm
#     - For each temperature treatment, 6 replicates
...
# In the exploratory plots above using a flexible GAM fit, the relationship looked
# mostly unimodal. Given this and the small sample size, a second-order polynomial
# should capture these trends well, while being the most parsimonious approach.
# 
# Including a random effect of Mesocosm can account for different starting levels
# of POC in the tanks. The time-response shape of POC in the tanks will be fit in an identical
# manner, which should be okay since the tanks had broadly similar curves.
# Ideally we would allow for random slopes as well, but do not have sufficient data
...
# Main result: POC trajectory over time differs significantly between temperatures
...
# CO2:Chlorophyll a Ratio ===================
## Data exploration =============
warm_curves_CO2_chla <- ggplot() +
   geom_point(data = warm_data,
              aes(x = DOY,
                  y = CO2_chla,
                  fill = Mesocosm_ID),
   ...
   ylab(NULL) +
   ylim(0, 4e06) +
   theme(legend.position = 'none'); warm_curves_CO2_chla

cold_curves_CO2_chla <- ggplot() +
   geom_point(data = cold_data,
              aes(x = DOY,
                  y = CO2_chla,
                  fill = Mesocosm_ID),
   ...
   ylab('POC:Chlorophyll a Ratio') +
   ylim(0, 4e06) +
   theme(legend.position = 'none'); cold_curves_CO2_chla

combined_curves_CO2_chla <- ggpubr::ggarrange(cold_curves_CO2_chla, warm_curves_CO2_chla); combined_curves_CO2_chla
ggsave(combined_curves_CO2_chla, filename = 'figures/combined_curves_CO2_chla.jpg', width = 12)

## Overall temporal trend of POC:Chlorophyll a Ratio =============
model_CO2_chla <- lmer(CO2_chla ~ Temperature * poly(DOY, degree = 2) + (1|Mesocosm), 
                       data = data) 
# Singular fit, this could happen if there was very little variation between Mesocosms.
simulationOutput <- simulateResiduals(fittedModel = model_CO2_chla)
testResiduals(simulationOutput)
plot(simulationOutput)

summary_CO2_chla <- as.data.frame(summary(model_CO2_chla)[['coefficients']]) %>% 
   mutate(response = 'POC:Chlorophyll a Ratio') %>% 
   rownames_to_column('coefficient')
anova_CO2_chla <- car::Anova(model_CO2_chla) %>% 
   mutate(response = 'POC:Chlorophyll a Ratio') %>% 
   rownames_to_column('coefficient')

### Plot model results =============
# Fitted DOY effect
newdata_DOY_CO2_chla <- expand.grid(
   DOY = seq(min(data$DOY), max(data$DOY), length.out = 100),
   Temperature = c('cold', 'warm')
)

preds_CO2_chla_DOY <- predict(model_CO2_chla, newdata = newdata_DOY_CO2_chla, se.fit = TRUE, re.form = NA)

newdata_CO2_chla_DOY <- newdata_DOY_CO2_chla %>%
   mutate(
      fit    = (preds_CO2_chla_DOY$fit),
      lower  = (preds_CO2_chla_DOY$fit - 1.96 * preds_CO2_chla_DOY$se.fit),
      upper  = (preds_CO2_chla_DOY$fit + 1.96 * preds_CO2_chla_DOY$se.fit)
   )

jpeg('figures/model_result_CO2_chla.jpg', res = 200, width = 10, height = 7, units = 'in')
ggplot() +
   geom_point(data = data, aes(x = DOY, y = CO2_chla, col = Temperature), cex = 2, alpha = 0.3) +
   geom_ribbon(data = newdata_CO2_chla_DOY, aes(x = DOY, ymin = lower, ymax = upper, fill = Temperature), alpha = 0.3) +
   geom_line(data = newdata_CO2_chla_DOY, aes(x = DOY, y = fit, col = Temperature), size = 1.2) +
   scale_color_manual(values = c('blue', 'red')) +
   scale_fill_manual(values = c('blue', 'red')) +
   labs(x = "DOY",
        y = "Predicted POC:Chlorophyll a Ratio")
...
newdata_CO2_chla_DOY$response <- 'POC:Chlorophyll a Ratio'

## Temporal trajectory analysis of POC:Chlorophyll a Ratio =============
### Aggregate the data =============
summary_data_CO2_chla <- data %>%
   group_by(Temperature, Treatment) %>%
   group_split() %>%
   map_dfr(summarize_tank, var = 'CO2_chla')

### Fit models =============
summary_results_CO2_chla <- summary_data_CO2_chla %>%
   select(Temperature, peak_fit, time_to_peak_fit, cum_pred_fitted) %>%   
   pivot_longer(-c(Temperature), names_to = "metric", values_to = "value") %>%
   group_by(metric) %>%
   summarize(model = list(lm(value ~ Temperature, data = cur_data())), .groups = "drop") %>%
...
### Plot the results =============
predictions_CO2_chla <- map_dfr(c("cum_pred_fitted", "peak_fit", "time_to_peak_fit"), function(m) {
   mod <- lm(as.formula(paste(m, "~ Temperature")), data = summary_data_CO2_chla)
   newdata <- tibble(Temperature = temp_levels)
...
p_values_CO2_chla <- summary_results_CO2_chla %>%
   filter(term == "Temperaturewarm") %>%
   select(metric, p.value) %>% 
   mutate(p.value = ifelse(p.value < 0.001, 'p < 0.001', paste0('p = ', p.value)))

jpeg('figures/temp_result_CO2_chla.jpg', res = 200, width = 10, height = 7, units = 'in')
ggplot(predictions_CO2_chla, aes(x = Temperature, y = estimate, group = metric)) +
   geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.5) +
   geom_point(pch = 23, size = 5, fill = 'coral') +
   facet_wrap(~ metric, scales = "free_y") +
   labs(
